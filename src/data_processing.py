...
class DataProcessor:
    ...
    def load_data(self, usecsols):
        try:
            self.rating_df = pd.read_csv(
                self.input_file, low_memory=True, usecols=usecsols
            )
    ...
    def filteruser(self, min_rating=400):
        try:
            ...
            self.rating_df = self.rating_df[
                self.rating_df["user_id"].isin(n_rating[n_rating >= min_rating].index)
            ]
    ...
    def enc_data(self):
        try:
            ...
            self.user2user_encoded = {
                x: i for i, x in enumerate(unqiue_user_id)
            }  # Encoding to eliminate gaps
            ...
            self.anime2anime_encoded = {
                x: i for i, x in enumerate(unqiue_anime_id)
            }  # Encoding to eliminate gaps
    ...
    def split_data(self, test_size=1000, random_state=43):
        try:
            self.rating_df = self.rating_df.sample(frac=1, random_state=43).reset_index(
                drop=True
            )
    ...
    def process_anime_data(self):
        try:
            ...
            df.sort_values(
                by="Score", ascending=False, na_position="last", inplace=True
            )
