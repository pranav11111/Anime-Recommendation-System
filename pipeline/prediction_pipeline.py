...
def hybrid_recom(user_id, user_weight=0.5, content_weight=0.5):
    ...
    similar_users = find_similar_users(
        int(user_id), USER_WEIGHTS_PATH, USER2USER_ENCODED, USER2USER_DECODED
    )
    user_pref = get_user_preferences(int(user_id), RATING_DF, DF)
    user_rec_anime = get_user_recommendations(
        similar_users, user_pref, DF, SYNOPSIS, RATING_DF
    )
    ...
    for anime in user_rec_anime_list:
        similar_content = find_similar_animes(
